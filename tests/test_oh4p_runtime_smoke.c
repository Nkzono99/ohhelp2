...
run_default_particle_path(void *pbuf, int maxlocalp, int rank, int *pbase) {
  ...
  mapped = oh4p_map_particle_to_subdomain(&particles[0], 0, 0);
  assert(mapped == rank);
...
run_weighted_load_path(void *pbuf, int rank, int *sdid, int *pbase,
  ...
  if (expect_secondary_unsupported)
    (void)oh4p_transbound(OH_MODE_REBALANCE_SECONDARY, 0);
...
main(int argc, char **argv) {
  ...
  maxlocalp = oh4p_max_local_particles(2, 20, 1, 1);
  assert(maxlocalp > 0);
  oh4p_init(&sdid, 1, 20, &totalp, &pbuf, &pbase, maxlocalp, NULL, &nbor,
            pcoord, &sdoms, scoord, 1, bcond, &bounds, (int*)ftypes,
            cfields, (int*)ctypes, &fsizes, 0, 0, 0);
  assert(sdid != NULL);
  assert(totalp != NULL);
  assert(pbuf != NULL);
  assert(pbase != NULL);

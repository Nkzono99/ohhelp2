...
- Obsolete Level-2 default-context particle exchange wrappers were removed
  after Level-3/4 callers switched to `*_state()` entry points.
...
- Level-3 default `S_particle` mapping callbacks are now installed through the
  active `oh_state` particle adapter mirror instead of writing the
  process-global adapter directly.

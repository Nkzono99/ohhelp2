...
contains
  subroutine run_particle_mutation_path(parts, maxlocalp_value, rank_value)
    ...
    mapped_value = oh4p_inject_particle(injected, 0)
    ...
    call oh4p_remove_mapped_particle(parts(injected_slot), 0, 1)
    if (parts(injected_slot)%nid /= -1) stop 21
    mapped_value = oh4p_remap_particle_to_subdomain(parts(injected_slot), &
                                                    0, 1)

...
finalize_injected_particles_state(struct oh_state *state) {
  ...
  int *counts, *hist, *offsets, *cursor;
  ...
  offsets = (int*)mem_alloc(sizeof(int), nclass, "FinalizeOffsets");
  cursor = (int*)mem_alloc(sizeof(int), nclass, "FinalizeCursor");
  for (i=0; i<nclass; i++) counts[i] = offsets[i] = cursor[i] = 0;
  ...
  total = 0;
  for (s=0; s<ns; s++) {
    offsets[s] = cursor[s] = total;
    total += counts[s];
  }
  state->primary_parts = total;
  for (s=0; s<ns; s++) {
    t = ns+s;
    offsets[t] = cursor[t] = total;
    total += counts[t];
  }
